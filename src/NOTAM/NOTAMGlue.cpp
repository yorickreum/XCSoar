...
AppendJsonDouble(std::string &out, double value)
{
  ...
  std::size_t trimmed = std::min(static_cast<std::size_t>(length),
                                 sizeof(buffer) - 1u);
...
static constexpr const char *NOTAM_CACHE_LOG_TOKEN = "<NOTAM_CACHE>";
  ...
  for (const auto &notam : impl->current_notams) {
    try {
      ...
      if (airspace_ptr) {
        ...
        if (main_text.empty() || !ValidateUTF8(main_text)) {
          if (!main_text.empty()) {
            LogFormat("NOTAM: Invalid UTF-8 in text for NOTAM '%s', using "
                      "identifier instead",
                      notam.number.c_str());
          }

          main_text = !notam.number.empty() ? notam.number : "NOTAM";
        }
        if (main_text.length() > NOTAM_TEXT_TRUNCATE_LENGTH) {
          main_text.resize(TruncateStringUTF8(
            std::string_view{main_text},
            NOTAM_TEXT_TRUNCATE_LENGTH - 3));
          main_text += "...";
        }
                            ...
                            const GeoPoint &location) const
{
  try {
    ...
    auto file_path = GetNOTAMCacheFilePath();
    LogFormat("NOTAM: Saving NOTAM cache: %s", NOTAM_CACHE_LOG_TOKEN);
