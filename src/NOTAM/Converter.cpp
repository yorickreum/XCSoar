                       ...
                       Airspaces &airspaces)
{
  try {
    ...
    switch (notam.geometry.type) {
      ...
      default:
        LogFormat("NOTAM: Unknown geometry type %d for NOTAM %s/%s, skipping",
    ...
    if (top.altitude < 0) {
      top.altitude = 20000; // Default to a very high ceiling: 20,000 m MSL
    ...
    LogFormat("NOTAM: Failed to convert NOTAM %s/%s: %s",
