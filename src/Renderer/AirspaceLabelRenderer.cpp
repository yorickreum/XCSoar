...
MakeNotamLabelText(const AbstractAirspace &airspace)
{
...
{
  return canvas.GetFontHeight() + Layout::GetTextPadding() + 2;
}
                                    ...
                                    LabelBlock *label_block) noexcept
{
  ...
  if (draw_notam_labels) {
    ...
    StaticArray<NotamLabelCluster, 64> clusters;
    const unsigned cluster_distance = GetNotamClusterDistance();

    for (const auto &i : airspaces->QueryWithinRange(projection.GetGeoScreenCenter(),
                                                     projection.GetScreenDistanceMeters())) {
      const AbstractAirspace &airspace = i.GetAirspace();
      if (!visible(airspace) || !IsNotamAirspace(airspace))
        continue;
      ...
      for (auto it = clusters.begin(), end = clusters.end(); it != end; ++it)
        if (MatchesNotamCluster(*it, *pos, cluster_distance)) {
          cluster = it;
          break;
        }
    ...
    for (const auto &cluster : clusters)
      DrawNotamCluster(canvas, cluster.anchor, cluster, mode,
                       projection.GetScreenRect(), label_block);
  }
}
